...
exports.submitMatchResult = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.warn('[submitMatchResult] Failed to parse league structure:', e.message);
    }
    ...
    if (req.body.isWalkover === 'true' || req.body.isWalkover === true) {
      ...
      // A draw is blocked ONLY if allowDraw is explicitly false, OR a noDrawRule method is configured, OR it's a knockout format
      ...
      if (adjP1 === adjP2 && snookerShouldBlockDraw && !winnerId) {
        ...
        } else if (isKnockoutFormat && !autoWinnerId && details && Array.isArray(details)) {
          // Cascading auto-resolution for knockout formats if rules didn't catch it
          let p1Total = 0, p2Total = 0;
          let p1Max = 0, p2Max = 0;
          details.forEach(f => {
            p1Total += parseInt(f.player1Score) || parseInt(f.player1BallsPotted) || 0;
            p2Total += parseInt(f.player2Score) || parseInt(f.player2BallsPotted) || 0;
            const b1 = parseInt(f.player1Break !== undefined ? f.player1Break : f.player1HighestBreak) || 0;
            const b2 = parseInt(f.player2Break !== undefined ? f.player2Break : f.player2HighestBreak) || 0;
            if (b1 > p1Max) p1Max = b1;
            if (b2 > p2Max) p2Max = b2;
          });
          if (p1Total > p2Total) autoWinnerId = actualPlayer1Id;
          else if (p2Total > p1Total) autoWinnerId = actualPlayer2Id;
          else if (p1Max > p2Max) autoWinnerId = actualPlayer1Id;
          else if (p2Max > p1Max) autoWinnerId = actualPlayer2Id;
        }
        if (autoWinnerId) {
      ...
      if (adjP1 === adjP2 && poolShouldBlockDraw && !winnerId) {
        ...
        if (matchRules.noDrawRule === 'mostPoints' && details && Array.isArray(details)) {
          ...
          let p1Max = 0, p2Max = 0;
          details.forEach(r => {
            p1Total += parseInt(r.player1Score) || parseInt(r.player1BallsPotted) || 0;
            p2Total += parseInt(r.player2Score) || parseInt(r.player2BallsPotted) || 0;
            const b1 = parseInt(r.player1Break) || 0;
            const b2 = parseInt(r.player2Break) || 0;
            if (b1 > p1Max) p1Max = b1;
            if (b2 > p2Max) p2Max = b2;
    ...
    if ((noDrawRule !== 'none' || isKnockoutFormat) && !finalWinnerId && !isWalkover) {
